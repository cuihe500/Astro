...
创建表单保留可见 label、字段级错误、提交中禁用。删除项目使用原生 `<dialog>` 二次确认，明确“仅空项目可删除”；确认和取消均可键盘操作，图标复用 Lucide 并保持可访问名称。现有响应式布局、颜色 token、反馈组件和 CSS 体系继续复用，不加运行时依赖或新设计系统；页面交互测试使用 Vitest 支持的 `jsdom` 作为仅开发依赖。
...
- Web：验证项目响应解码、路由构造、无项目空状态、创建/删除交互及项目上下文传播；`jsdom` 只提供测试 DOM，不进入运行时构建。
