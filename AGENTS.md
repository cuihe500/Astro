...
19. **命令使用**: 必须使用 Makefile 中的目标，不得随意手拼命令。可用目标：`make build`（编译到 `bin/astro`）、`make run`（本地运行）、`make test`（全部测试）、`make lint`（golangci-lint 静态检查）、`make swagger`（生成 Swagger 文档）、`make clean`（清理产物）
